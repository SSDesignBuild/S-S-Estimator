...
function parseLabReportText(text, profile) {
  ...
  const matches = Object.keys(LAB_DEFAULTS)
...
function buildSchedulePreviewItem(schedule = {}, injections = [], referenceDate = todayInput()) {
  ...
  const lastInjection = matchingInjections[0] || null;
  const injectedToday = !!lastInjection && sameDay(lastInjection.date, referenceDate);
  ...
  const anchorTime = lastInjection?.time || fallbackTime;
  const anchorDateTime = parseDateTime(anchorDate, anchorTime);
  const progressBase = cadenceHours > 0 && anchorDateTime
    ? Math.max(0, (Date.now() - anchorDateTime.getTime()) / (cadenceHours * 3600000))
    : 0;
  ...
  return {
    ...
    cycleProgress: clamp(progressBase * 100, 0, 100),
    injectedToday
  };
}
...
function buildScheduleDueEntriesForRange(schedule = {}, injections = [], startDate, endDate) {
  ...
  if (cadenceHours > 0) {
    cursor = matchingInjections[0]
      ? addHoursToDateTime(matchingInjections[0].date, matchingInjections[0].time || fallbackTime, cadenceHours)
      : addHoursToDateTime(schedule.startDate, fallbackTime, cadenceHours);
  } else {
    cursor = { date: schedule.startDate, time: fallbackTime, dateTime: parseDateTime(schedule.startDate, fallbackTime) };
  }
...
function buildProtocolGuidance(profile, phase) {
  ...
  const peptideContext = profile.usingPeptides && profile.peptideCategory ? `${profile.peptideCategory} context active.` : "";
  return {
    headline:
      phaseType === "Blast"
...
export default function App() {
  ...
  const dynamicSignalPool = useMemo(() => {
    ...
    return pool;
  }, [welcomeTagline, currentDaypart, activePhase, todayWeekStatus, todayCardioEntries.length, nutritionIdeaSignal, signalSeed, recentFoodNames, nutritionTargets?.protein, nutritionTargets?.carbs, todayFoodTotals.protein, todayFoodTotals.carbs, plannedWorkoutMetaByDate, workoutCompletionByDate]);
  ...
  const movementCuesCard = (
    <div style={styles.card}>
      ...
      <div style={{ ...styles.weekDetailGrid, marginTop: 14 }}>
        ...
        <div style={styles.weekDetailColumn}>
          <div style={styles.listTitle}>Cardio cue</div>
          <div style={styles.historyMiniCard}>
...
${buildWorkoutPlan(prev.groups, workoutTemplateSelection, activePhase?.type || phaseForm.type || "Maintenance")}`
  ...
  function resetCardioLogForm() {
    ...
    setCardioForm({
      ...
      minutes: todayCardioPlan?.kind === "cardio" ? String(todayCardioPlan.minutes || "") : String(lastUsedCardioMinutes || ""),
      notes: ""
    });
  }
            ...
            <div style={styles.stackLayout}>
              ...
              <div style={styles.card}>
                ...
                <div style={styles.contextSummary}>
                  ...
                  <div style={styles.smallText}>
                    {schedulePreview[0]
                      ? `Next checkpoint: ${fmtDate(schedulePreview[0].nextDate)} • ${schedulePreview[0].nextTime || schedulePreview[0].time}`
                    ...
                    <div key={x.id} style={styles.listCard}>
                      <div style={styles.listTop}>
                        <div>
                          ...
                          <div style={styles.smallText}>
                            {x.dosage} {x.unit} • every {x.everyDays} days • {x.nextTime || x.time}
                          </div>
                          <div style={styles.smallText}>
                            {x.injectedToday
              ...
              >
                <div style={styles.formGrid}>
                  ...
                  <div style={styles.importPanelCard}>
                    ...
                    <div style={styles.smallText}>
                      Parsing runs in the browser. Text PDFs are fastest. High-resolution exports work best; screenshots from the original report usually read better than compressed camera images. Images and scanned PDFs use OCR and may take a little longer.
                    </div>
            ...
            {editing.table === "body_metrics" ? (
              <div style={styles.actionRow}>
                <button
                  ...
                  onClick={async () => {
                    ...
                    const ok = await updateRow("body_metrics", editing.id, { ...bodyPayload }, setBodyMetrics, bodyMetrics);
                    if (!ok) return;
              ...
              <button
                ...
                onClick={async () => {
                  const nextId = makeId();
                  const { visceralFatLevel, ...bodyPayload } = bodyForm;
                  const ok = await saveRow("body_metrics", { id: nextId, ...bodyPayload }, setBodyMetrics, bodyMetrics);
                  if (!ok) return;
        ...
        >
          <div style={styles.formGrid}>
            ...
            <div style={{ ...styles.libraryCard, padding: 12, gap: 8 }}>
              <div style={styles.listTop}>
                <div>
                  <div style={styles.listTitle}>Fast logging</div>
                  <div style={styles.smallText}>Repeat yesterday or tap a saved preset. Build new presets or searchable custom foods in Quick Add below.</div>
                </div>
                <div style={styles.badge}>{savedMeals.length} saved</div>
              </div>
              <div style={{ ...styles.actionRowSmall, marginTop: 8 }}>
                <button
                  type="button"
              ...
              {savedMeals.length ? (
                <div style={{ marginTop: 8, display: "grid", gap: 8 }}>
                  {savedMeals.slice(0, 4).map((meal) => (
                    <div key={meal.id} style={styles.listCard}>
                      <div style={styles.listTop}>
                        <div>
                          <div style={styles.listTitle}>{meal.name}</div>
                          <div style={styles.smallText}>{meal.calories} cal • P {meal.protein} • C {meal.carbs} • F {meal.fats}</div>
                        </div>
                        <div style={styles.actionRowSmall}>
                          <button
                            type="button"
                          ...
                          <button type="button" style={styles.deleteButton} onClick={() => removeMealPreset(meal.id)}>Remove</button>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            >
              <div style={styles.formGrid}>
                ...
                <div style={styles.smallText}>
                  Use this when you already know the macros. Log it once, save it as a one-tap meal preset, or add it to your searchable food library.
                </div>
                ...
                {editing.table === "custom_foods" ? (
                  ...
                  <div style={styles.actionRow}>
                    ...
                    <button
                      ...
                      style={styles.secondaryButton}
                      onClick={() => {
                        const meal = buildQuickMacroPayload();
                        if (!quickMacroHasAnyValue(meal)) {
                          setStatus("Add at least one macro value first.");
                          return;
                        }
                        saveMealPreset(meal);
                        setStatus(`${meal.name} saved.`);
                      }}
                    >
                      Save as meal preset
                    </button>
        ...
        >
          <div style={styles.formGrid}>
            ...
            <div>
              ...
              <select
                style={styles.input}
                value={cardioForm.minutes && CARDIO_MINUTE_OPTIONS.includes(Number(cardioForm.minutes || 0)) ? String(cardioForm.minutes) : (lastUsedCardioMinutes && CARDIO_MINUTE_OPTIONS.includes(Number(lastUsedCardioMinutes || 0)) ? String(lastUsedCardioMinutes) : "custom")}
                onChange={(e) => {
                  const nextValue = e.target.value;
              ...
              >
                {CARDIO_MINUTE_OPTIONS.map((minutes) => (
